...
error0 = np.sum(np.abs(jv(0,x)- J0))/len(J0)
error3 = np.sum(np.abs(jv(3,x)- J3))/len(J3)
error5 = np.sum(np.abs(jv(5,x)- J5))/len(J5)

print(f"""Average Manhattan Error for n = 0: {error0}
Average Manhattan Error for n = 3: {error3}
Average Manhattan Error for n = 5: {error5}""")